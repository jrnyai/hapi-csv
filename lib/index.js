  ...
  register: (server, options) => {
    ...
    server.ext("onPreStart", (srv) => {
      return srv.table().forEach((route) => {
        if (route.settings.response.schema) {
          internals.routeMap.set(
            internals.createRouteMethodString(route.path, route.method),
            route.settings.response.schema
          );
        }
      });
    });
    ...
    server.ext("onPreResponse", async (request, h) => {
      ...
      if (
        !(
          preferredType &&
          internals.routeMap.has(
            internals.createRouteMethodString(
              request.route.path,
      ...
      const dynamicHandlerObject =
        request.route.settings.plugins["hapi-csv"] || {};
      ...
      const schema = internals.routeMap.get(
        internals.createRouteMethodString(
          request.route.path,
          request.route.method
        )
      );
      const { headerQueryMap, dataset } = internals.processSchema(
        schema,
        resolvedSchemasObject,
      ...
      if (!Array.isArray(dataset) && !(dataset === Object(dataset))) {
        ...
        return response
          .type(
            `${preferredType}; charset=${response.settings.charset}; header=present;`
          )
      ...
      if (
        ...
        res.setHeader(
          "Content-type",
          `${preferredType}; charset=utf-8; header=present;`
        );
...
internals.processSchema = (schema, dynamicSchemas, dataset, resultKey) => {
  ...
  const headerQueryArray = internals.parseSchema(
    schemaDescription,
    dynamicSchemas
  );
...
internals.parseSchema = (
  ...
  if (joiSchema.type === "object") {
    ...
    let children = childrenKeys.map((key) =>
      internals.parseSchema(
        joiSchema.keys[key],
        dynamicSchemas,
        key,
        path !== undefined ? `${path}.${key}` : key
      )
    );
  ...
  if (joiSchema.type === "array") {
    ...
    const parsedItem = internals.parseSchema(
      item,
      dynamicSchemas,
      keyName,
      path,
    ...
    if (keyName && parsedItem) {
      ...
      for (let i = 0; i < internals.maximumElementsInArray; ++i) {
        prefixedItemArray.push(
          parsedItem.map((headerQuery) => {
